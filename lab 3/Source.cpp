...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	wnd.hIcon = LoadIcon(hInstance, IDI_APPLICATION);//MAKEINTRESOURCE(IDI_ICON2));
	wnd.hCursor = LoadCursor(hInstance, IDC_ARROW);//MAKEINTRESOURCE(IDC_CURSOR1));
	wnd.hbrBackground = CreateSolidBrush(RGB(42, 200, 34));//(HBRUSH)GetStockObject(WHITE_BRUSH);
	wnd.lpszMenuName = NULL;
	wnd.lpszClassName = _TEXT("Window");
	wnd.hIconSm = LoadIcon(hInstance, IDI_APPLICATION);//MAKEINTRESOURCE(IDI_ICON2));
	wnd.cbClsExtra = 0;
	wnd.cbWndExtra = 0;
